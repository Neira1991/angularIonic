...
app.controller('consultorController', ['$rootScope', '$scope', '$template', 'User', '$filter', '$state', 'Registro', '$sce', '$timeout', 'PagesStatic', function($rootScope, $scope, $template, User, $filter, $state, Registro, $sce, $timeout, PagesStatic) {
  ...
  $scope.changeValues = function(){
    ...
    $scope.registro3.get({
      ...
      console.log($scope.TotalDesembolsosConSeguro);
      console.log("scope.TotalDesembolsosConSeguro");
      console.log($scope.TotalDesembolsos);
      console.log("scope.TotalDesembolsos");
      console.log($scope.listaCotizaciones.length);
      console.log("$scope.listaCotizaciones.length");
      var allDesembolsos =  $scope.TotalDesembolsos + $scope.TotalDesembolsosConSeguro
      console.log(allDesembolsos+"->allDesembolsos");
      $scope.percentPenetracion = Math.floor(($scope.TotalDesembolsosConSeguro / allDesembolsos)* 100);
      ...
      $scope.percentEfectividad = Math.floor(($scope.TotalDesembolsos / ($scope.listaCotizaciones.length + $scope.TotalDesembolsos)) * 100);
  ...
  $scope.registros = function() {
    ...
    console.log($scope.registro.getUsers({
      // "IDUs": 1
      "IDUs": $rootScope.user.data.id
    }));
    $scope.registro.get({
      ...
      $scope.listaCotizaciones = r;
      console.log($scope.TotalDesembolsos+"->$scope.TotalDesembolsos");
      console.log($scope.TotalDesembolsosConSeguro+"$scope.TotalDesembolsosConSeguro");
      var allDesembolsos =  $scope.TotalDesembolsos + $scope.TotalDesembolsosConSeguro
      $scope.percentPenetracion = Math.floor((($scope.TotalDesembolsosConSeguro) / ($scope.TotalDesembolsosConSeguro/allDesembolsos))*100);
      $scope.percentEfectividad = Math.floor((($scope.TotalDesembolsos) / ($scope.listaCotizaciones.length + $scope.TotalDesembolsos)) * 100);
      $scope.percentPenetracion = isNaN($scope.percentPenetracion) ? 0 : $scope.percentPenetracion
      $scope.percentEfectividad = isNaN($scope.percentEfectividad) ? 0 : $scope.percentEfectividad
  ...
  $scope.sendMail = function(){
    ...
    $scope.datosPrestamoSeleted.category = mailhtml($scope.datosPrestamoSeleted.category);
    $scope.mailObject = {
      asesor: $scope.user,
      cliente: $scope.datosPrestamoSeleted.PersonalArray,
      head: {
        titulo1: "Resultados 1",
        ...
        if (form.$valid) {
            ...
            user.login({
                ...
                if ( result == 1 ) {
                  ...
                  $scope.user.data.img = $sce.trustAsHtml($scope.user.data.img);
                  var data={
                    "user": $rootScope.user.data.id.toString(),
                    "date": Session.getTime()
                  };
                  console.log("Session="+Session.getTime());


                  /*estadisticas de usuario*/
                  $api.post('wp-services/userStats.php', data)
...
app.factory('Session', ['$state', '$api', '$q', function($state, $api, $q) {
  ...
  var Session = {
    getTime: function(){
      ...
      var time = now.toISOString().substring(0, 10)+" "+ now.getHours()+":"+now.getMinutes()+":00";
      return time;
    },
...
app.service('$api', ['$http', '$q', 'config', function($http, $q, config) {
    ...
    this.post = function(endpoint, data) {
        ...
        console.log("url que envia Post= "+JSON.stringify(data));
...
function mailhtml(variablestring){
  switch (variablestring) {
    case "Préstamo Personal":
      console.log("1");
      variablestring = "Pr&eacute;stamo Personal";
      return variablestring;
      break;
    case "Préstamo de Vivienda":
    console.log("2");
      variablestring = "Pr&eacute;stamo de Vivienda";
      return variablestring;
      break;
    case "Préstamo de Vehículo Tradicional":
    console.log("3");
      variablestring = "Pr&eacute;stamo de Veh&iacute;culo Tradicional";
      return variablestring;
      break;
    case "Préstamo de Vehículo 14 Cuotas":
    console.log("4");
      variablestring = "Pr&eacute;stamo de Veh&iacute;culo 14 Cuotas";
      return variablestring;
      break;
    case "Préstamo de Vehículo Premium":
    return variablestring;
    console.log("5");
      variablestring = "Pr&eacute;stamo de Veh&iacute;culo Premium";
      return variablestring;
      break;
    default:
